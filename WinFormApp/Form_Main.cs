...
消除方块
Version 7.1.17000.4720.R16.180613-0000

This file is part of 消除方块

消除方块 is released under the GPLv3 license
* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
...
namespace WinFormApp
{
    public partial class Form_Main : Form
    {
        ...
        private static readonly string RootDir_Product = Environment.SystemDirectory.Substring(0, 1) + @":\ProgramData\AppConfig\eliminate"; // 根目录：此产品。
        ...
        private static readonly List<Version> OldVersionList = new List<Version> // 兼容的版本列表，用于从最新的兼容版本迁移配置设置。
        {
            ...
            new Version(7, 1, 17000, 60),
            new Version(7, 1, 17000, 284),
            new Version(7, 1, 17000, 293),
            new Version(7, 1, 17000, 476),
            new Version(7, 1, 17000, 699),
            new Version(7, 1, 17000, 918),
            new Version(7, 1, 17000, 3290),
            new Version(7, 1, 17000, 3989),
            new Version(7, 1, 17000, 4225),
        ...
        private static readonly string URL_GitHub_Base = @"https://github.com/chibayuki/Eliminate"; // 此项目在 GitHub 的 URL。
        ...
        private static readonly SizeF RangeAspectRatio = new SizeF(8F, 5F); // 界面布局的宽高比。
        private const Int32 RangeWidthStep = 4; // 界面布局宽度步长.

        private static readonly Size Range_MAX = new Size(64, 40); // 最大界面布局。
        private static readonly Size Range_MIN = new Size(16, 10); // 最小界面布局。
        private Size Range = new Size(24, 15); // 当前界面布局（以元素数为单位）。

        //

        private const Int32 DifficultyLevel_MIN = -100; // 最小难度等级。
        private const Int32 DifficultyLevel_MAX = 100; // 最大难度等级。
        private Int32 DifficultyLevel = 0; // 当前难度等级。难度等级越大，难度越高。

        //

        private bool AppendRowWhenClickBottom = true; // 当点击最底行时是否立即添加一行方块。
        ...
        private const Int32 CAPACITY = 64; // 元素矩阵容量的平方根。
        ...
        {
            public static readonly Color LvFinish = Color.FromArgb(36, 143, 89); // 过关颜色。
            public static readonly Color GameOver = Color.FromArgb(235, 71, 99); // 失败颜色。
            public static readonly Color Pause = Color.FromArgb(242, 147, 13); // 暂停颜色。
        }
        ...
        private const Int64 TOP_LEVEL = (Int64)1E15; // 最高等级。

        //

        private Int32 AutoAppendTimeSpan = 0; // 自动追加的时间间隔（毫秒）。
        ...
        private struct Record // 记录。
        {
            ...
            public double Score; // 已完成等级总分数。
            public Int64 Level; // 已完成等级。
            public double CurrentScore; // 当前等级分数。
            public Int64 CurrentSurplus; // 完成当前等级仍需消除的元素数。
        }
        ...
        private void LoadedEvents(object sender, EventArgs e)
        {
            ...
            ComboBox_Range.SelectedIndexChanged -= ComboBox_Range_SelectedIndexChanged;

            for (int w = Range_MIN.Width; w <= Range_MAX.Width; w += RangeWidthStep)
            {
                ComboBox_Range.Items.Add(w + " × " + (Int32)(w / RangeAspectRatio.Width * RangeAspectRatio.Height));
            }

            ComboBox_Range.SelectedIndex = ComboBox_Range.Items.IndexOf(Range.Width + " × " + Range.Height);

            ComboBox_Range.SelectedIndexChanged += ComboBox_Range_SelectedIndexChanged;

            //

            CheckBox_AppendRowWhenClickBottom.CheckedChanged -= CheckBox_AppendRowWhenClickBottom_CheckedChanged;

            CheckBox_AppendRowWhenClickBottom.Checked = AppendRowWhenClickBottom;
        ...
        private void ResizeEvents(object sender, EventArgs e)
        {
            ...
            Panel_AppendRowWhenClickBottom.Width = Panel_Tab_Options.Width - Panel_AppendRowWhenClickBottom.Left * 2;
        ...
        private void FormStateChangedEvents(object sender, EventArgs e)
        {
            ...
            if (Me.FormState == Com.WinForm.FormState.Minimized && Timer_AutoAppend.Enabled)
            {
        ...
        private void ThemeColorChangedEvents(object sender, EventArgs e)
        {
            ...
            Label_ThisRecordVal_Lv.ForeColor = Label_BestRecordVal_Lv.ForeColor = Me.RecommendColors.Text.ToColor();
            ...
            Label_Range.ForeColor = Label_DifficultyLevel.ForeColor = Label_AppendRowWhenClickBottom.ForeColor = Label_ThemeColor.ForeColor = Label_AntiAlias.ForeColor = Me.RecommendColors.Text_INC.ToColor();

            ComboBox_Range.BackColor = Me.RecommendColors.MenuItemBackground.ToColor();
            ComboBox_Range.ForeColor = Me.RecommendColors.MenuItemText.ToColor();
            ...
            CheckBox_AppendRowWhenClickBottom.ForeColor = Me.RecommendColors.Text.ToColor();
            ...
            PictureBox_Interrupt.Image = (Timer_AutoAppend.Enabled ? InterruptImages.Pause : InterruptImages.Resume);
        ...
        private void Panel_AppendRowWhenClickBottom_Paint(object sender, PaintEventArgs e)
        {
            //
            // Panel_AppendRowWhenClickBottom 绘图。
            //
                ...
                if (new FileInfo(ConfigFilePath).Length > 0)
                {
                    ...
                    try
                    {
                        ...
                        if (Fields.Length == 2)
                        {
                            ...
                            {
                                R.Width = (R.Width - Range_MIN.Width) / RangeWidthStep * RangeWidthStep + Range_MIN.Width;
                                R.Height = (Int32)(R.Width / RangeAspectRatio.Width * RangeAspectRatio.Height);
                    ...
                    try
                    {
                        string SubStr = RegexInt.Replace(Com.Text.GetIntervalString(Cfg, "<DifficultyLevel>", "</DifficultyLevel>", false, false), string.Empty);
                    ...
                    if (Com.Text.GetIntervalString(Cfg, "<AppendRowWhenClickBottom>", "</AppendRowWhenClickBottom>", false, false).Contains((!AppendRowWhenClickBottom).ToString()))
                    {
                        AppendRowWhenClickBottom = !AppendRowWhenClickBottom;
                    }
        ...
        private void SaveConfig()
        {
            ...
            Cfg += "<AppendRowWhenClickBottom>" + AppendRowWhenClickBottom + "</AppendRowWhenClickBottom>";
                        ...
                        while (SubStr.IndexOf("(") != -1 && SubStr.IndexOf(")") != -1)
                        {
                            try
                            {
                                ...
                                if (Fields.Length == 4)
                                {
                                    ...
                                    string StrLv = RegexUint.Replace(Fields[i++], string.Empty);
                                    Rec.Level = Convert.ToInt64(StrLv);

                                    if ((Rec.Range.Width >= Range_MIN.Width && Rec.Range.Width <= Range_MAX.Width) && (Rec.Range.Height >= Range_MIN.Height && Rec.Range.Height <= Range_MAX.Height) && Rec.Score >= 0 && (Rec.Level >= 0 && Rec.Level <= TOP_LEVEL))
                                    {
        ...
        private void SaveUserData()
        {
            ...
            if (BestRecord.Score < ThisRecord.Score || (BestRecord.Score == ThisRecord.Score && BestRecord.Level > ThisRecord.Level))
            {
            ...
            for (int w = Range_MIN.Width; w <= Range_MAX.Width; w++)
            {
                for (int h = Range_MIN.Height; h <= Range_MAX.Height; h++)
                {
                    ...
                    if (Rec.Score > 0)
                    {
                        Str += "(" + Rec.Range.Width + "," + Rec.Range.Height + "," + Rec.Score + "," + Rec.Level + ")";
                    }
                }
            }
            Str += "]</BestRecord>";
        ...
        private void LoadLastGame()
        {
            ...
            if (File.Exists(RecordFilePath))
            {
                ...
                if (FInfo.Length > 0)
                {
                    ...
                    try
                    {
                        string SubStr = RegexInt.Replace(Com.Text.GetIntervalString(Str, "<DifficultyLevel>", "</DifficultyLevel>", false, false), string.Empty);

                        Int32 DL = Convert.ToInt32(SubStr);

                        if (DL >= DifficultyLevel_MIN && DL <= DifficultyLevel_MAX)
                        {
                            Record_Last.DifficultyLevel = DL;
                        }
                    }
                    catch { }
                    ...
                    try
                    {
                        ...
                        if (Fields.Length == 4)
                        {
                            ...
                            string StrCScore = RegexFloatExp.Replace(Fields[i++], string.Empty);
                            Rec.CurrentScore = Convert.ToDouble(StrCScore);

                            string StrCSp = RegexUint.Replace(Fields[i++], string.Empty);
                            Rec.CurrentSurplus = Convert.ToInt64(StrCSp);

                            if (Rec.Score >= 0 && (Rec.Level >= 0 && Rec.Level <= TOP_LEVEL))
                            {
                                Record_Last.Score = Rec.Score;
                                Record_Last.Level = Rec.Level;
                                Record_Last.CurrentScore = Rec.CurrentScore;
                                Record_Last.CurrentSurplus = Rec.CurrentSurplus;
                            }
                        }
                    }
                    catch { }
        ...
        private void SaveLastGame()
        {
            ...
            Str += "<Element>[";
            if (ThisRecord.CurrentSurplus > 0)
            {
            ...
            Str += "<Record>(" + ThisRecord.Score + "," + ThisRecord.Level + "," + ThisRecord.CurrentScore + "," + ThisRecord.CurrentSurplus + ")</Record>";
        ...
        private Point ElementMatrix_GetIndex(Point P)
        {
            ...
            try
            {
                ...
                Point A = new Point((Int32)Math.Floor((double)dP.X / ElementSize), Range.Height - 1 - (Int32)Math.Floor((double)dP.Y / ElementSize));
        ...
        private static readonly Color[] ElementColor = new Color[] { Color.Crimson, Color.DarkOrange, Color.ForestGreen, Color.BlueViolet }; // 元素颜色系列。
        ...
        private void ElementMatrix_DrawInRectangle(Int32 E, Rectangle Rect, bool HighLight, bool PresentNow)
        {
            ...
            if (Rect.Width < BmpRect.Width || Rect.Height < BmpRect.Height)
            {
                ...
                if (!Timer_AutoAppend.Enabled && ThisRecord.CurrentSurplus > 0)
                {
        ...
        private void ElementMatrix_DrawAtPoint(Point A, bool PresentNow)
        {
            ...
            if (ElementMatrix_IndexValid(A))
            {
                ...
                ElementMatrix_DrawInRectangle(E, Rect, (E > 0 && A == GameUIPointedIndex && A.Y > 0), PresentNow);
            }
        }
            ...
            if (Panel_Environment.Visible && (Panel_Environment.Width > 0 && Panel_Environment.Height > 0))
            {
                ...
                for (int X = 0; X < Range.Width; X++)
                {
                    for (int Y = 0; Y < Range.Height; Y++)
                    {
                        ...
                        Rectangle Rect = new Rectangle(new Point(X * ElementSize, (Range.Height - 1 - Y) * ElementSize), new Size(ElementSize, ElementSize));
                ...
                if (!Timer_AutoAppend.Enabled)
                {
                    ...
                    else
                    {
                        ...
                        {
                            StringText = "已完成 Lv " + (ThisRecord.Level + 1);
                        }
                    }
            ...
            if (Panel_Environment.Visible && (Panel_Environment.Width > 0 && Panel_Environment.Height > 0))
            {
                if (ElementMatrix_IndexValid(A))
                {
                    ...
                    Com.Animation.Frame Frame = (frameId, frameCount, msPerFrame) =>
                    {
                        ...
                        Rectangle Rect = new Rectangle(new Point(A.X * ElementSize + (ElementSize - RectSize) / 2, (Range.Height - 1 - A.Y) * ElementSize + (ElementSize - RectSize) / 2), new Size(RectSize, RectSize));
        ...
        private void ElementMatrix_AnimatePresentAt(List<Point> A, bool ClearBmp, Record OldRecord, Record NewRecord)
        {
            //
            // 以动画效果同时呈现元素矩阵中由索引数组指定的所有元素。A：索引列表；ClearBmp：在呈现之前是否首先清除绘图；OldRecord，NewRecord：元素矩阵中所有元素在平移之前与之后的记录。
            //
            ...
            if (Panel_Environment.Visible && (Panel_Environment.Width > 0 && Panel_Environment.Height > 0))
            {
                ...
                Com.Animation.Frame Frame = (frameId, frameCount, msPerFrame) =>
                {
                    ...
                    if (OldRecord.CurrentScore != NewRecord.CurrentScore || OldRecord.CurrentSurplus != NewRecord.CurrentSurplus)
                    {
                        double Pct_Rec = (frameId == frameCount ? 1 : (double)frameId / frameCount);

                        ThisRecord.CurrentScore = Math.Round(OldRecord.CurrentScore * (1 - Pct_Rec) + NewRecord.CurrentScore * Pct_Rec);
                        ThisRecord.CurrentSurplus = (Int64)Math.Round(OldRecord.CurrentSurplus * (1 - Pct_Rec) + NewRecord.CurrentSurplus * Pct_Rec);
        ...
        private void ElementMatrix_AnimatePresentAt(List<Point> A, bool ClearBmp)
        {
            ...
            if (Panel_Environment.Visible && (Panel_Environment.Width > 0 && Panel_Environment.Height > 0))
            {
                ElementMatrix_AnimatePresentAt(A, ClearBmp, ThisRecord, ThisRecord);
            }
        }
                    ...
                    for (int X = 0; X < Range.Width; X++)
                    {
                        for (int Y = 0; Y < Range.Height; Y++)
                        {
                            ...
                            if (E != 0 && OldIndex[X, Y] != new Point(X, Y))
                            {
                                ...
                                Rectangle Rect = new Rectangle(new Point((Int32)((X * (1 - N) + OldIndex[X, Y].X * N) * ElementSize), (Int32)((Range.Height - 1 - (Y * (1 - N) + OldIndex[X, Y].Y * N)) * ElementSize)), new Size(ElementSize, ElementSize));
                    ...
                    for (int X = 0; X < Range.Width; X++)
                    {
                        for (int Y = 0; Y < Range.Height; Y++)
                        {
                            ...
                            if (E != 0 && OldIndex[X, Y] != new Point(X, Y))
                            {
                                ...
                                Rectangle Rect = new Rectangle(new Point((Int32)((X + Math.Sign(OldIndex[X, Y].X - X) * N) * ElementSize), (Int32)((Range.Height - 1 - (Y + Math.Sign(OldIndex[X, Y].Y - Y) * N)) * ElementSize)), new Size(ElementSize, ElementSize));
        ...
        private void ElementMatrix_RecursivelyAccounting(Point A)
        {
            //
            ...
            if (E != 0)
            {
                if (!AdjacentEqualityIndexList.Contains(A))
                {
                    AdjacentEqualityIndexList.Add(A);
                }

                Point L = new Point(A.X - 1, A.Y), R = new Point(A.X + 1, A.Y), B = new Point(A.X, A.Y - 1), T = new Point(A.X, A.Y + 1);

                if ((L.Y >= 1 && L.X >= 0) && ElementMatrix_GetValue(L) == E && !AdjacentEqualityIndexList.Contains(L))
                {
                    AdjacentEqualityIndexList.Add(L);

                    IDListTmp.Add(L);
                }

                if ((R.Y >= 1 && R.X < Range.Width) && ElementMatrix_GetValue(R) == E && !AdjacentEqualityIndexList.Contains(R))
                {
                    AdjacentEqualityIndexList.Add(R);

                    IDListTmp.Add(R);
                }

                if (B.Y >= 1 && ElementMatrix_GetValue(B) == E && !AdjacentEqualityIndexList.Contains(B))
                {
                    AdjacentEqualityIndexList.Add(B);

                    IDListTmp.Add(B);
                }

                if (T.Y < Range.Height && ElementMatrix_GetValue(T) == E && !AdjacentEqualityIndexList.Contains(T))
                {
                    AdjacentEqualityIndexList.Add(T);
        ...
        private void ElementMatrix_LogicalEliminate(Point A)
        {
            //
            // 逻辑消除。消除元素矩阵中所有与指定索引处的元素相邻相等的元素，向下对齐，水平向中合并。A：索引。
            //

            if (ElementMatrix_IndexValid(A) && A.Y >= 1)
            {
                ElementMatrix_RecursivelyAccounting(A);

                if (AdjacentEqualityIndexList.Count >= 3)
                {
                    ...
                    NewRecord.CurrentScore = Math.Floor(ThisRecord.CurrentScore + (Math.Sqrt(ThisRecord.Level) + (double)AdjacentEqualityIndexList.Count / 3) * AdjacentEqualityIndexList.Count);
                    NewRecord.CurrentSurplus = Math.Max(0, ThisRecord.CurrentSurplus - AdjacentEqualityIndexList.Count);

                    ElementMatrix_AnimatePresentAt(AdjacentEqualityIndexList, false, OldRecord, NewRecord);
                    ...
                    for (int X = 0; X < Range.Width; X++)
                    {
                        if (!NeedAlignment)
                        {
                            bool NullColumn = true;

                            for (int Y = Range.Height - 1; Y >= 1; Y--)
                            {
                                if (NullColumn)
                                {
                        ...
                        for (int X = 0; X < Range.Width; X++)
                        {
                            ...
                            for (int Y = 1; Y < Range.Height; Y++)
                            {
                                if (ElementMatrix_GetValue(X, Y) != 0)
                                {
                                    ...
                                    OldIndex[X, Column.Count] = new Point(X, Y);
                                }
                            }

                            for (int Y = 0; Y < Column.Count; Y++)
                            {
                                ElementMatrix_Add(new Point(X, Y + 1), Column[Y]);
                            }
                        }
                    ...
                    bool NeedConsolidation = false, NullMatrix = true;
                    Int32 Left = -1, Right = -1;

                    for (int X = 0; X < Range.Width; X++)
                    {
                        if (ElementMatrix_GetValue(X, 1) != 0)
                        {
                            NullMatrix = false;
                    ...
                    if (!NullMatrix)
                    {
                        for (int X = Range.Width - 1; X >= Left; X--)
                        {
                            if (ElementMatrix_GetValue(X, 1) != 0)
                            {
                                NullMatrix = false;
                    ...
                    if (NeedConsolidation)
                    {
                        ...
                        for (int X = 0; X < Range.Width; X++)
                        {
                            if (ElementMatrix_GetValue(X, 1) != 0)
                            {
                                ...
                                for (int Y = 1; Y < Range.Height; Y++)
                                {
                                    if (ElementMatrix_GetValue(X, Y) != 0)
                                    {
                                        Matrix[IndexList.Count - 1, Y] = ElementMatrix_GetValue(X, Y);
                        ...
                        for (int X = 0; X < IndexList.Count; X++)
                        {
                            for (int Y = 1; Y < Range.Height; Y++)
                            {
                                if (Matrix[X, Y] != 0)
                                {
                                    ElementMatrix_Add(new Point((Range.Width - IndexList.Count) / 2 + X, Y), Matrix[X, Y]);

                                    OldIndex[(Range.Width - IndexList.Count) / 2 + X, Y] = new Point(IndexList[X], Y);
                                }
                                else
                                {
                                    break;
                ...
                AdjacentEqualityIndexList.Clear();
            }
        }

        private void ElementMatrix_LogicalAppend(bool Present)
        {
            //
            // 向元素矩阵逻辑添加一个元素，元素的值按照由难度等级决定的概率产生。Present：是否呈现此元素。
            //

            if (ElementMatrix_GetValue(Range.Width - 1, 0) != 0)
            {
                for (int X = 0; X < Range.Width; X++)
                {
                    for (int Y = Range.Height - 2; Y >= 0; Y--)
                    {
                        if (ElementMatrix_GetValue(X, Y) != 0)
                        {
                            ElementMatrix_Add(new Point(X, Y + 1), ElementMatrix_GetValue(X, Y));
                            ElementMatrix_RemoveAt(new Point(X, Y));
                        }
                    }
                }
            ...
            for (int X = 0; X < Range.Width; X++)
            {
                if (ElementMatrix_GetValue(X, 0) == 0)
                {
                    if (DifficultyLevel < 0)
                    {
                        Int32 E = Com.Statistics.RandomInteger(ElementColor.Length) + 1;

                        while (X == 0 ? (E != ElementMatrix_GetValue(0, 1) && E != ElementMatrix_GetValue(1, 1)) : (X == Range.Width - 1 ? (E != ElementMatrix_GetValue(Range.Width - 2, 0) && E != ElementMatrix_GetValue(Range.Width - 2, 1) && E != ElementMatrix_GetValue(Range.Width - 1, 1)) : (E != ElementMatrix_GetValue(X - 1, 0) && E != ElementMatrix_GetValue(X - 1, 1) && E != ElementMatrix_GetValue(X, 1) && E != ElementMatrix_GetValue(X + 1, 1))))
                        {
                            if (Com.Statistics.RandomInteger(DifficultyLevel_MIN, 0) <= Math.Max(DifficultyLevel_MIN, DifficultyLevel))
                            {
                                break;
                            }

                            E = Com.Statistics.RandomInteger(ElementColor.Length) + 1;
                        }

                        ElementMatrix_Add(new Point(X, 0), E);
                    }
                    else if (DifficultyLevel > 0)
                    {
                        ...
                        while (X == 0 ? (E == ElementMatrix_GetValue(0, 1) || E == ElementMatrix_GetValue(1, 1)) : (X == Range.Width - 1 ? (E == ElementMatrix_GetValue(Range.Width - 2, 0) || E == ElementMatrix_GetValue(Range.Width - 2, 1) || E == ElementMatrix_GetValue(Range.Width - 1, 1)) : (E == ElementMatrix_GetValue(X - 1, 0) || E == ElementMatrix_GetValue(X - 1, 1) || E == ElementMatrix_GetValue(X, 1) || E == ElementMatrix_GetValue(X + 1, 1))))
                        {
                            if (Com.Statistics.RandomInteger(1, DifficultyLevel_MAX + 1) >= Math.Min(DifficultyLevel_MAX, DifficultyLevel))
                            {
                                break;
                            }
                    ...
                    {
                        ElementMatrix_Add(new Point(X, 0), Com.Statistics.RandomInteger(ElementColor.Length) + 1);
                    }

                    if (Present)
                    {
                        ElementMatrix_AnimatePresentAt(new Point(X, 0));
                    }
        ...
        private CycData CD_AutoAppend = new CycData(); // Timer_AutoAppend 计时周期数据。

        private Int32 AutoAppend_Cyc = 0; // Timer_AutoAppend 计时周期。

        private void Timer_AutoAppend_Tick(object sender, EventArgs e)
        {
            //
            // Timer_AutoAppend。
            //
        ...
        private void AutoAppendStart()
        {
            ...
            Timer_AutoAppend.Enabled = true;
        }

        private void AutoAppendWorkOnce()
        {
            //
            // 自动追加进行一次工作。
            //

            CD_AutoAppend.Update();

            //

            ThisGameTime += TimeSpan.FromMilliseconds(CD_AutoAppend.DeltaMS);
            TotalGameTime += TimeSpan.FromMilliseconds(CD_AutoAppend.DeltaMS);

            AutoAppend_Cyc += (Int32)CD_AutoAppend.DeltaMS;

            if (AutoAppend_Cyc >= AutoAppendTimeSpan)
            {
                AutoAppend_Cyc = 0;

                ElementMatrix_LogicalAppend(true);
        ...
        private void AutoAppendStop()
        {
            //
            // 自动追加停止。
            //

            Timer_AutoAppend.Enabled = false;
        }
        ...
        private void SelectMissionFromLevel(Int64 Lv, bool Present)
        {
            //
            // 选择关卡。Lv，等级；Present：是否呈现位图。
            //

            if (Lv >= 1)
            {
                Lv = Math.Min(TOP_LEVEL, Lv);

                ThisRecord.Level = Lv - 1;
                ThisRecord.CurrentScore = 0;
                ThisRecord.CurrentSurplus = (Lv + 10) * Range.Width;

                ElementMatrix_Initialize();

                for (int i = 0; i <= Math.Min(Range.Height - 6, Lv + 4) * Range.Width; i++)
                {
                    ElementMatrix_LogicalAppend(false);
                }

                RepaintCurBmp();

                AutoAppendTimeSpan = (Int32)Math.Max(1, Math.Max(200 - DifficultyLevel, 700 - DifficultyLevel - Lv * 20));
        ...
        private void Judgement()
        {
            ...
            CeilingElementIndexList.Clear();

            for (int X = 0; X < Range.Width; X++)
            {
                if (ElementMatrix_GetValue(X, Range.Height - 1) != 0)
                {
                    CeilingElementIndexList.Add(X);
                }
            }
            ...
            if (!GameIsOver)
            {
                if (CeilingElementIndexList.Count > 0 && ElementMatrix_GetValue(Range.Width - 1, 0) != 0)
                {
                    GameIsOver = true;
                ...
                else if (ThisRecord.CurrentSurplus <= 0)
                {
                    AutoAppendStop();
        ...
        private enum InterruptActions { NULL = -1, StartNew, Continue, Pause, Resume, NextLevel, Restart, ExitGame, CloseApp, COUNT }; // 中断动作枚举。
        ...
        private void Interrupt(InterruptActions IA)
        {
            ...
            switch (IA)
            {
                ...
                case InterruptActions.Continue: // 继续上次的游戏。
                    {
                        ...
                        if (Record_Last.CurrentSurplus <= 0)
                        {
                            SelectMissionFromLevel(Record_Last.Level + 1, true);
                        }
                        else
                        {
                            SelectMissionFromLevel(Record_Last.Level + 1, false);

                            ThisRecord.CurrentScore = Record_Last.CurrentScore;
                            ThisRecord.CurrentSurplus = Record_Last.CurrentSurplus;
                ...
                case InterruptActions.NextLevel: // 晋级。
                    {
                        ThisRecord.Range = Range;
                        ThisRecord.DifficultyLevel = DifficultyLevel;
                        ThisRecord.Level += 1;
                        ThisRecord.Score += ThisRecord.CurrentScore;

                        SaveUserData();

                        SelectMissionFromLevel(ThisRecord.Level + 1, true);
        ...
        private void PictureBox_Interrupt_MouseEnter(object sender, EventArgs e)
        {
            ...
            if (!GameIsOver)
            {
                ToolTip_InterruptPrompt.SetToolTip(PictureBox_Interrupt, (Timer_AutoAppend.Enabled ? "暂停" : (ThisRecord.CurrentSurplus > 0 ? "恢复" : "下一关")));
            }
        }
        ...
        private void PictureBox_Interrupt_Click(object sender, EventArgs e)
        {
            ...
            if (!GameIsOver)
            {
                ...
                else
                {
                    ...
                    {
                        Interrupt(InterruptActions.NextLevel);
                    }
                }
            }
        }
        ...
        private void PictureBox_ExitGame_MouseEnter(object sender, EventArgs e)
        {
            ...
            ToolTip_InterruptPrompt.SetToolTip(PictureBox_ExitGame, (!GameIsOver && ThisRecord.Score + ThisRecord.CurrentScore > 0 ? "保存并退出" : "退出"));
        }
        ...
        private void Panel_Environment_MouseDown(object sender, MouseEventArgs e)
        {
            ...
            if (Timer_AutoAppend.Enabled && e.Button == MouseButtons.Left)
            {
                Point A = ElementMatrix_GetIndex(Com.Geometry.GetCursorPositionOfControl(Panel_Environment));
                ...
                if (ElementMatrix_IndexValid(A))
                {
                    if (A.Y >= 1)
                    {
                        ...
                        {
                            ElementMatrix_LogicalEliminate(A);
                        }
                    }
                    else
                    {
                        if (AppendRowWhenClickBottom)
                        {
                            Int32 MaxY = 0;
                            ...
                            if (MaxY <= Range.Height - 2)
                            {
                                ...
                                for (int X = 0; X < Range.Width; X++)
                                {
                                    if (ElementMatrix_GetValue(X, 0) == 0)
                                    {
                                        BottomNew.Add(new Point(X, 0));

                                        ElementMatrix_LogicalAppend(false);
                                    }
                                }

                                ElementMatrix_AnimatePresentAt(BottomNew, false);
        ...
        private void UpdateCurBmp()
        {
            ...
            Rectangle Rect_Current = new Rectangle(Rect_Total.Location, new Size((Int32)Math.Max(2, Math.Min(1, (double)(((ThisRecord.Level + 1) + 10) * Range.Width - ThisRecord.CurrentSurplus) / (((ThisRecord.Level + 1) + 10) * Range.Width)) * Rect_Total.Width), Rect_Total.Height));
            ...
            while (FloorCount < Range.Width && ElementMatrix_GetValue(FloorCount, 0) != 0)
            {
                FloorCount++;
            }

            double TimeElapsedPct = (YMaxID * Range.Width + FloorCount + Math.Min(1, (double)AutoAppend_Cyc / AutoAppendTimeSpan)) / (Range.Height * Range.Width);

            Rectangle Rect_TimeRemaining = new Rectangle(Rect_Total.Location, new Size((GameIsOver || ThisRecord.CurrentSurplus <= 0) ? Rect_Total.Width : (Int32)Math.Max(2, Math.Min(1, TimeElapsedPct) * Rect_Total.Width), 5));

            Color Color_TimeRemaining = Color.Empty;

            if (Timer_AutoAppend.Enabled)
            {
                if (CeilingElementIndexList.Count == 0)
                {
                    Color_TimeRemaining = Com.ColorManipulation.GetGrayscaleColor(Me.RecommendColors.Border).ToColor();
                }
                else
                {
                    double TimeRemainingSec = (Range.Height * Range.Width) * (AutoAppendTimeSpan * 0.001) * (1 - TimeElapsedPct);

                    Color_TimeRemaining = Com.ColorManipulation.BlendByRGB(GlobalColors.GameOver, Com.ColorManipulation.GetGrayscaleColor(Me.RecommendColors.Border).ToColor(), Math.Abs(TimeRemainingSec * 2 - ((Int32)Math.Ceiling(TimeRemainingSec * 2) / 2) * 2));
                }
            }
            else
            {
                if (GameIsOver)
                {
                    Color_TimeRemaining = GlobalColors.GameOver;
                }
                else
                {
                    if (ThisRecord.CurrentSurplus > 0)
                    {
                        Color_TimeRemaining = GlobalColors.Pause;
                    }
                    else
                    {
                        Color_TimeRemaining = GlobalColors.LvFinish;
                    }
                }
            }
            ...
            string StringText_Score = Math.Max(0, ThisRecord.Score + ThisRecord.CurrentScore).ToString();
            ...
            string StringText_Lv = "等级: ", StringText_Lv_Val = "Lv " + Math.Max(0, ThisRecord.Level + 1);
            Color StringColor_Lv = Me.RecommendColors.Text.ToColor(), StringColor_Lv_Val = Me.RecommendColors.Text_INC.ToColor();
            Font StringFont_Lv = new Font("微软雅黑", 12F, FontStyle.Regular, GraphicsUnit.Point, 134), StringFont_Lv_Val = new Font("微软雅黑", 12F, FontStyle.Bold, GraphicsUnit.Point, 134);
            RectangleF StringRect_Lv = new RectangleF(), StringRect_Lv_Val = new RectangleF();
            StringRect_Lv.Size = CurBmpGrap.MeasureString(StringText_Lv, StringFont_Lv);
            StringRect_Lv_Val.Size = CurBmpGrap.MeasureString(StringText_Lv_Val, StringFont_Lv_Val);

            string StringText_Surplus = "剩余: ", StringText_Surplus_Val = Math.Max(0, ThisRecord.CurrentSurplus).ToString();
            Color StringColor_Surplus = Me.RecommendColors.Text.ToColor(), StringColor_Surplus_Val = Me.RecommendColors.Text_INC.ToColor();
            Font StringFont_Surplus = new Font("微软雅黑", 12F, FontStyle.Regular, GraphicsUnit.Point, 134), StringFont_Surplus_Val = new Font("微软雅黑", 12F, FontStyle.Bold, GraphicsUnit.Point, 134);
            RectangleF StringRect_Surplus = new RectangleF(), StringRect_Surplus_Val = new RectangleF();
            StringRect_Surplus.Size = CurBmpGrap.MeasureString(StringText_Surplus, StringFont_Surplus);
            StringRect_Surplus_Val.Size = CurBmpGrap.MeasureString(StringText_Surplus_Val, StringFont_Surplus_Val);

            RegionSize_L = StringRect_Score.Size;
            RegionSize_R = new SizeF(Math.Max(StringRect_Lv.Width + StringRect_Lv_Val.Width, StringRect_Surplus.Width + StringRect_Surplus_Val.Width), 0);
            ...
            StringRect_Lv_Val.Location = new PointF(RegionRect.Right - StringRect_Lv_Val.Width, (RegionRect.Height / 2 - StringRect_Lv_Val.Height) / 2);
            StringRect_Lv.Location = new PointF(StringRect_Lv_Val.X - StringRect_Lv.Width, (RegionRect.Height / 2 - StringRect_Lv.Height) / 2);

            Com.Painting2D.PaintTextWithShadow(CurBmp, StringText_Lv, StringFont_Lv, StringColor_Lv, StringColor_Lv, StringRect_Lv.Location, 0.1F, AntiAlias);
            Com.Painting2D.PaintTextWithShadow(CurBmp, StringText_Lv_Val, StringFont_Lv_Val, StringColor_Lv_Val, StringColor_Lv_Val, StringRect_Lv_Val.Location, 0.1F, AntiAlias);

            StringRect_Surplus_Val.Location = new PointF(RegionRect.Right - StringRect_Surplus_Val.Width, RegionRect.Height / 2 + (RegionRect.Height / 2 - StringRect_Surplus_Val.Height) / 2);
            StringRect_Surplus.Location = new PointF(StringRect_Surplus_Val.X - StringRect_Surplus.Width, RegionRect.Height / 2 + (RegionRect.Height / 2 - StringRect_Surplus.Height) / 2);

            Com.Painting2D.PaintTextWithShadow(CurBmp, StringText_Surplus, StringFont_Surplus, StringColor_Surplus, StringColor_Surplus, StringRect_Surplus.Location, 0.1F, AntiAlias);
            Com.Painting2D.PaintTextWithShadow(CurBmp, StringText_Surplus_Val, StringFont_Surplus_Val, StringColor_Surplus_Val, StringColor_Surplus_Val, StringRect_Surplus_Val.Location, 0.1F, AntiAlias);
        }
            ...
            set
            {
                ...
                switch (_FunctionAreaTab)
                {
                    case FunctionAreaTabs.Start:
                        {
                            if (!(ElementIndexList_Last.Count == 0 && Record_Last.Score <= 0 && Record_Last.Level <= 0 && Record_Last.CurrentScore <= 0 && Record_Last.CurrentSurplus <= 0))
                            {
                    ...
                    case FunctionAreaTabs.Record:
                        {
                            if (BestRecord.Score == 0)
                            {
                                ...
                                Label_ThisRecordVal_Lv.Text = "等级: 无";
                                Label_BestRecordVal_Score.Text = "无记录";
                                Label_BestRecordVal_Lv.Text = "等级: 无";
                            }
                            else
                            {
                                ...
                                Label_ThisRecordVal_Lv.Text = "等级: Lv " + ThRec.Level;
                                Label_BestRecordVal_Score.Text = BestRecord.Score.ToString();
                                Label_BestRecordVal_Lv.Text = "等级: Lv " + BestRecord.Level;
                            }
        ...
        private void PaintScore(PaintEventArgs e)
        {
            ...
            Label_ThisRecordVal_Lv.Left = Math.Max(0, Math.Min(Panel_Score.Width - Label_ThisRecordVal_Lv.Width, (Panel_Score.Width / 2 - Label_ThisRecordVal_Lv.Width) / 2));
            Label_ThisRecordVal_Lv.Top = Rect_This.Y - 5 - Label_ThisRecordVal_Lv.Height;
            ...
            Label_ThisRecordVal_Score.Top = Label_ThisRecordVal_Lv.Top - Label_ThisRecordVal_Score.Height;

            Label_BestRecordVal_Lv.Left = Math.Max(0, Math.Min(Panel_Score.Width - Label_BestRecordVal_Lv.Width, Panel_Score.Width / 2 + (Panel_Score.Width / 2 - Label_BestRecordVal_Lv.Width) / 2));
            Label_BestRecordVal_Lv.Top = Rect_Best.Y - 5 - Label_BestRecordVal_Lv.Height;
            ...
            Label_BestRecordVal_Score.Top = Label_BestRecordVal_Lv.Top - Label_BestRecordVal_Score.Height;
        }
        ...
        {
            //
            // ComboBox_Range 选中项索引改变。
            //

            Range = new Size(Range_MIN.Width + ComboBox_Range.SelectedIndex * RangeWidthStep, (Int32)((Range_MIN.Width + ComboBox_Range.SelectedIndex * RangeWidthStep) / RangeAspectRatio.Width * RangeAspectRatio.Height));
        }

        // 难度。

        private const Int32 DifficultyLevelMouseWheelStep = 5; // 难度等级的鼠标滚轮调节步长。
        ...
        private void UpdateDLTrb()
        {
            ...
            using (Graphics DLTrbBmpGrap = Graphics.FromImage(DLTrbBmp))
            {
                ...
                Label_DifficultyLevel_Val.Text = (DifficultyLevel > 0 ? "+" : string.Empty) + DifficultyLevel;
        ...
        private void CheckBox_AppendRowWhenClickBottom_CheckedChanged(object sender, EventArgs e)
        {
            //
            // CheckBox_AppendRowWhenClickBottom 选中状态改变。
            //

            AppendRowWhenClickBottom = CheckBox_AppendRowWhenClickBottom.Checked;
        }
